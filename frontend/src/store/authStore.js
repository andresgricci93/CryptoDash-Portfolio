...
export const useAuthStore = create((set) => ({
  ...
  validateMagicLink: async (token) => {
    ...
    try {
      ...
      // Load user currency after magic link authentication
      const { useCurrencyStore } = await import('../store/currencyStore.js');
      console.log('About to load user currency after magic link...');
      await useCurrencyStore.getState().loadUserCurrency();
      console.log('User currency loaded after magic link');

      return response.data;
    } catch (error) {
      set({
        error: error.response?.data?.message || "Invalid magic link",
        isLoading: false,
        isAuthenticated: false
      });
      throw error;
